...
static volatile uint32_t _UART_BAUD = 0;
static volatile uint32_t _UART_LINE = 0;

static void _LineCoding2Reg(void)
{
    uint32_t u32Reg;
    ...
    if (u32Baud_Div > 0xFFFF) {
        _UART_BAUD = (UART_BAUD_MODE0 | UART_BAUD_MODE0_DIVIDER(__HXT, gLineCoding.u32DTERate));
    } else {
        _UART_BAUD = (UART_BAUD_MODE2 | u32Baud_Div);
    }
...
void RS232_LineCoding(void)
{
    ...
    UART2->FIFO = 0x3;
    // Set baudrate
    UART2->BAUD = _UART_BAUD;
    UART2->LINE = _UART_LINE;
...
void RS485_LineCoding(void)
{
    ...
    UART1->FIFO = 0x3;
    // Set baudrate
    UART1->BAUD = _UART_BAUD;
    UART1->LINE = _UART_LINE;
    // Re-enable UART interrupt
...
void RS232Monitor_LineCoding(void)
{
    ...
    UART1->BAUD = _UART_BAUD;
    UART1->LINE = _UART_LINE;
    UART3->BAUD = _UART_BAUD;
    UART3->LINE = _UART_LINE;
    // Re-enable UART interrupt
    UART1->FIFO = UART_FIFO_RFITL_14BYTES | UART_FIFO_RTSTRGLV_14BYTES;
    UART1->TOUT = (UART1->TOUT & ~UART_TOUT_TOIC_Msk) | (10);
    UART3->FIFO = UART_FIFO_RFITL_14BYTES | UART_FIFO_RTSTRGLV_14BYTES;
    UART3->TOUT = (UART3->TOUT & ~UART_TOUT_TOIC_Msk) | (10);
    UART1->INTEN = (UART_INTEN_TOCNTEN_Msk | UART_INTEN_RXTOIEN_Msk | UART_INTEN_RDAIEN_Msk);
    UART3->INTEN = (UART_INTEN_TOCNTEN_Msk | UART_INTEN_RXTOIEN_Msk | UART_INTEN_RDAIEN_Msk);
}
...
void RS232Monitor_TransferData(void)
{
    ...
    while ((UART1->FIFOSTS & UART_FIFOSTS_RXEMPTY_Msk) == 0) {	//RX fifo not empty
        /* Enqueue the character */
        comRbuf[comRtail++] = 0x01;
        comRbuf[comRtail++] = UART1->DAT;
        comRbytes += 2;
    }

    /* Receiver FIFO threshold level is reached or Rx time out */
    while ((UART3->FIFOSTS & UART_FIFOSTS_RXEMPTY_Msk) == 0) {	//RX fifo not empty
        /* Enqueue the character */
        comRbuf[comRtail++] = 0x03;
        comRbuf[comRtail++] = UART3->DAT;
        comRbytes += 2;
    }
